...
async function loadPostDetail() {
    ...
    const slug = pathParts[1]; // post/slug-here
    
    try {
        showLoading();
        const data = await apiCall(`/posts/${slug}/`);
        // ✅ لو الـ slug اتغيّر في الـ backend بعد التعديل
        if (data.slug && data.slug !== slug) {
            // استبدال الرابط بدون refresh
            window.history.replaceState({}, '', `/post/${data.slug}/`);
        }
        ...
        document.getElementById('postHero').innerHTML = '<h1 style="color: var(--danger);">المقال غير موجود</h1>';
    } finally {
        hideLoading();
    }
}
...
// ========== Render Post Hero ==========
async function renderPostHero(post) {
    ...
    document.getElementById('postHero').innerHTML = `
        ...
        <div style="display: flex; align-items: center; gap: 2rem; flex-wrap: wrap;">
            ...
            <div style="display: flex; gap: 2rem; opacity: 0.9;">
                ...
                <span><i class="fas fa-comment"></i> ${post.comments_count || 0} تعليق</span>
            </div>
...
async function loadCategoriesWidget() {
    try {
        ...
        // data is already an array, no need for .results
        const categories = Array.isArray(data) ? data.slice(0, 8) : [];
        
        const container = document.getElementById('categoriesWidget');
        if (categories.length === 0) {
            container.innerHTML = '<p style="color: var(--gray); text-align: center;">لا توجد فئات</p>';
        } else {
